...
``Zend\Config\Config`` implements theCountableandIteratorinterfaces in order to facilitate simple access to configuration data. Thus, one may use the `count()`_ function and *PHP* constructs such as `foreach`_ with ``Zend\Config\Config`` objects.
