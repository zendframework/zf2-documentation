...
------------------
    ...
    As of 2.0.0beta3, you can use the ``StaticEventManager`` singleton as a ``SharedEventCollection`` . As such, you do not need to worry about where and how to get access to the ``SharedEventCollection`` ; it's globally available by simply callingStaticEventManager::getInstance().
...
What the above specifies is thatanyevent triggered will result in notification of this particular listener.
