...
To create a new *URI* from scratch, pass only the scheme to ``Zend_Uri::factory()`` 
At the time of writing, ``Zend_Uri`` only provides built-in support for the *HTTP* and *HTTPS* schemes.
. If an unsupported scheme is passed and no scheme-specific class is specified, a ``Zend_Uri_Exception`` will be thrown.
