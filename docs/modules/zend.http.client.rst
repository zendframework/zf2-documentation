...
**__constructstring $uri, array $config**
...
**setConfigConfig|array $config = array (
)**
...
**setAdapterZend\Http\Client\Adapter|string $adapter**


Load the connection adapter

While this method is not called more than one for a client, it isseperated from ->request() to preserve logic and readability
...
**setRequestZend\Http\Zend\Http\Request $request**
...
**setResponseZend\Http\Zend\Http\Response $response**
...
**setUristring|Zend\Http\Zend\Uri\Http $uri**
...
**setEncTypestring $encType, string $boundary**
...
**addCookieArrayIterator|SetCookie|string $cookie, string $value, string $domain, string $expire, string $path, boolean $secure = false, boolean $httponly = true**
...
**setHeadersHeaders|array $headers**
...
**setStreamstring|boolean $streamfile = true**
...
**setAuthstring $user, string $password, string $type = 'basic'**


Create a HTTP authentication "Authorization:" header according to thespecified user, password and authentication method.
...
**setFileUploadstring $filename, string $formname, string $data, string $ctype**


Set a file to upload (using a POST request)

Can be used in two ways:1. $data is null (default): $filename is treated as the name if a local file whichwill be read and sent. Will try to guess the content type using mime_content_type().2. $data is set - $filename is sent as the file name, but $data is sent as the filecontents and no file is read from the file system. In this case, you need tomanually set the Content-Type ($ctype) or it will default toapplication/octet-stream.
...
**removeFileUploadstring $filename**
...
**encodeFormDatastring $boundary, string $name, mixed $value, string $filename, array $headers = array (
)**
...
The request() method takes one optional parameter - the request method. This can be either ``GET`` , ``POST`` , ``PUT`` , ``HEAD`` , ``DELETE`` , ``TRACE`` , ``OPTIONS`` or ``CONNECT`` as defined by the *HTTP* protocol
