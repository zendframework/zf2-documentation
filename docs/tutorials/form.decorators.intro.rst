...
:ref:`Zend_Form <zend.form>` utilizes thedecoratorpattern in order to render elements and forms. Unlike the classic `decorator pattern`_ , in which you pass an object to a wrapping class, decorators in ``Zend_Form`` implement a `strategy pattern`_ , and utilize the metadata contained in an element or form in order to create a representation of it
