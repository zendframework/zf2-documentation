...
DocTypedeclarations are troublesome to memorize, and often essential to include in your document to ensure the browser properly renders your content. The ``doctype()`` view helper allows you to use simple string mnemonics to specify the desiredDocType; additionally, other helpers will query the ``doctype()`` helper to ensure the output generated conforms with the requestedDocType.
...
.. note::
    **InlineScript Variant**
    ...
    - Alternately, you may use the inlineScript() helper,
    - which is simply a variant on headScript(), and
