...
Les instances ``Zend\Session\Namespace`` fournissent l'API primaire pour manipuler les données de session dans
...
``Zend\Session\Namespace`` fournissent plus efficacement et plus solidement leur suite de fonctionnalités liées
à session.

Chaque instance de ``Zend\Session\Namespace`` correspond à une entrée dans le tableau de la superglobale
``$_SESSION``, où l'espace de noms est utilisée comme une clé.

   .. code-block:: php
      :linenos:

      $monNamespace = new Zend\Session\Namespace('monNamespace');
...
``$_SESSION`` ne correspondant pas aux instances de ``Zend\Session\Namespace``.
...
Si aucun espace de noms n'est spécifié lors de l'instanciation de ``Zend\Session\Namespace``, toutes les données
...
utilisons ``Zend\Session\Namespace``. L'exemple ci-dessous montre l'utilisation de cet espace de noms par défaut,
...
Quand de multiples modules utilisent des instances de ``Zend\Session\Namespace`` ayant différents espaces de noms,
chaque module obtient une encapsulation pour ses propres données de session. Le constructeur de
``Zend\Session\Namespace`` peut recevoir un paramètre facultatif ``$namespace``, qui permet aux développeurs la
...
.. code-block:: php
   ...
   $webServiceNamespace = new Zend\Session\Namespace('Un_Service_Web');
...
``Zend\Session\Namespace`` fournit une `interface IteratorAggregate`_ complète, incluant le support de
...
.. code-block:: php
   ...
   $unNamespace =
       new Zend\Session\Namespace('un_namespace_avec_des_donnes_presentes');
...
``Zend\Session\Namespace`` implémente ``__get()``, ``__set()``, ``__isset()``, et ``__unset()``. `Les méthodes
...
.. code-block:: php
   ...
   $namespace = new Zend\Session\Namespace();
