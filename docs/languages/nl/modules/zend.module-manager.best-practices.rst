...
Er zijn enkele aanbevelingen waar je rekening dient mee te houden als je een ZF2 module ontwikkelt:
...
- **Hou de ``init()`` en ``onBootstrap()`` methoden eenvoudig.** Wees conservatief met de acties die je uitvoert
  ...
  pagina request en kunnen beter geen zware acties uitvoeren. Als vuistregel kan je stellen dat het registreren van
  event listeners een geschikte taak is die kan uitgevoerd worden in deze twee methoden. Zo'n lichtgewicht taken
  ...
  van apllicatie resources aan te maken of te configureren zoals database connecties, een applicatie logger of een
  mailer. Zo'n taken zijn beter geschikt voor de ``ServiceManager`` van Zend Framework 2.
