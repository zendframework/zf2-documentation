...
Um eine E-Mail über SMTP zu versenden, muss ``Zend\Mail_Transport\Smtp`` erstellt und in ``Zend_Mail`` registriert
...
Die ``setDefaultTransport()`` Methode und der Konstruktor von ``Zend\Mail_Transport\Smtp`` sind nicht aufwendig.
