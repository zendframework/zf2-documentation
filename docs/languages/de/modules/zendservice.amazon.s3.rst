...
Bevor man mit ``ZendService\Amazon\S3`` beginnen kann, muß man einen Account registrieren. Sehen Sie bitte auf
...
Die Klasse ``ZendService\Amazon\S3`` bietet einen *PHP* Wrapper zum Amazon S3 REST Interface. Schauen Sie bitte in
...
``ZendService\Amazon\S3`` bietet die folgenden Funktionalitäten:
...
.. rubric:: Beispiel der Verwendung von ZendService\Amazon\S3
...
Da der ``ZendService\Amazon\S3`` Service eine Authentifizierung benötigt, sollte man seine Zugangsdaten (AWS
...
- ``removeBucket()`` entfernt den Bucket vom System. Der Bucket sollte leer sein damit er entfernt werden kann.
  ...
  .. rubric:: Beispiel für das Entfernen eines Buckets in ZendService\Amazon\S3
...
- ``getBuckets()`` gibt eine Liste der Namen aller Buckets zurück die einem Benutzer gehören.
  ...
  .. rubric:: Beispiel für das Auflisten der Buckets in ZendService\Amazon\S3
...
- ``putObject($object, $data, $meta)`` erstellt ein Objekt mit dem Namen ``$object`` (Sollte den Bucketnamen als
  ...
  **S3_ACL_HEADER**
     ...
     .. rubric:: Beispiel für ein öffentliches Objekt in ZendService\Amazon\S3
...
- ``getObjectsByBucket($bucket)`` gibt eine Liste der Objektschlüssel zurüc, die im Bucket enthalten sind.
  ...
  .. rubric:: Beispiel für die Auflistung eines ZendService\Amazon\S3 Objekts
...
Streaming <zend.http.client.streaming>` verwendet werden kann.
      ...
      .. rubric:: Beispiel für das Streamen von Daten mit ZendService\Amazon\S3
...
Zusätzlich zum oben beschriebenen Interface unterstützt ``ZendService\Amazon\S3`` das Arbeiten als Stream
Wrapper. Hierfür muß das Client-Objekt als Stream Wrapper registriert werden:

.. _zendservice.amazon.s3.streams.example:

.. rubric:: Beispiel für Streams mit ZendService\Amazon\S3
