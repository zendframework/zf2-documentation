...
``Zend_Service_Twitter`` bietet einen Client für die `RESTAPI von Twitter`_. ``Zend_Service_Twitter`` erlaubt es
...
``Zend_Service_Twitter`` implementiert einen *REST* Service und alle Methoden geben eine Instanz von
``Zend_Rest_Client_Result`` zurück.

``Zend_Service_Twitter`` ist in Untersektionen geteilt damit man einfachst erkennen kann welcher Typ von Aufrufen
angefragt wird.
...
Mit der Ausnahme des Holens der öffentlichen Zeitlinie benötigt ``Zend_Service_Twitter`` eine Authentifizierung
...
``Zend_Service_Twitter`` verwendet wird ist ``Zend_OAuth``.

.. _zend.service.twitter.authentication.example:

.. rubric:: Erstellung der Twitter Klasse

``Zend_Service_Twitter`` muss sich selbst authorisieren, durch den Benutzer, bevor es mit der Twitter API verwendet
...
``Zend_Oauth`` über ``Zend_Service_Twitter`` welche zu einem internen ``Zend_Oauth_Consumer`` Objekt weiterleitet.
Bitte sehen Sie in die Dokumentation von ``Zend_Oauth`` für ein vollständiges Beispiel dieses Workflows - man
kann alle dokumentierten Methoden von ``Zend_Oauth_Consumer`` auf ``Zend_Service_Twitter`` aufrufen inklusive der
Optionen des Constructors. Man kann ``Zend_Oauth`` auch direkt verwenden und nur den sich ergebenden Zugriffstoken
an ``Zend_Service_Twitter`` übergeben. Das ist der normale Workflow sobald man einen wiederverwendbaren
...
man sich für jede neue Instanz von ``Zend_Service_Twitter`` authorisieren). Man sollte im Kopf behalten dass die
...
Das folgende Beispiel demonstriert das Setup von ``Zend_Service_Twitter`` welchem ein bereits bereitgestellter
...
``Zend_Service_Twitter`` übergeben werden. Die Dokumentation von ``Zend_Oauth`` demonstriert den Workflow wenn
...
.. note::
   ...
   Die vorher gezeigte pre-OAuth Version von ``Zend_Service_Twitter`` erlaubte die Übergabe eines Benutzernamens
