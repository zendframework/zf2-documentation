...
Bevor man mit ``Zend_Service_Amazon_S3`` beginnen kann, muß man einen Account registrieren. Sehen Sie bitte auf
...
Die Klasse ``Zend_Service_Amazon_S3`` bietet einen *PHP* Wrapper zum Amazon S3 REST Interface. Schauen Sie bitte in
...
.. rubric:: Beispiel der Verwendung von Zend_Service_Amazon_S3
...
Da der ``Zend_Service_Amazon_S3`` Service eine Authentifizierung benötigt, sollte man seine Zugangsdaten (AWS
...
- ``removeBucket()`` entfernt den Bucket vom System. Der Bucket sollte leer sein damit er entfernt werden kann.
  ...
  .. rubric:: Beispiel für das Entfernen eines Buckets in Zend_Service_Amazon_S3
...
- ``getBuckets()`` gibt eine Liste der Namen aller Buckets zurück die einem Benutzer gehören.
  ...
  .. rubric:: Beispiel für das Auflisten der Buckets in Zend_Service_Amazon_S3
...
- ``putObject($object, $data, $meta)`` erstellt ein Objekt mit dem Namen ``$object`` (Sollte den Bucketnamen als
  ...
  **S3_ACL_HEADER**
     ...
     .. rubric:: Beispiel für ein öffentliches Objekt in Zend_Service_Amazon_S3
...
- ``getObjectsByBucket($bucket)`` gibt eine Liste der Objektschlüssel zurüc, die im Bucket enthalten sind.
  ...
  .. rubric:: Beispiel für die Auflistung eines Zend_Service_Amazon_S3 Objekts
...
Diese Methode gibt einen ``Zend_Http_Response_Stream`` zurück, welcher wie im Kapitel :ref:`HTTP Client Daten
Streaming <zend.http.client.streaming>` verwendet werden kann.
      ...
      .. rubric:: Beispiel für das Streamen von Daten mit Zend_Service_Amazon_S3
...
Zusätzlich zum oben beschriebenen Interface unterstützt ``Zend_Service_Amazon_S3`` das Arbeiten als Stream
Wrapper. Hierfür muß das Client-Objekt als Stream Wrapper registriert werden:

.. _zend.service.amazon.s3.streams.example:

.. rubric:: Beispiel für Streams mit Zend_Service_Amazon_S3
