...
Методы Metadata::get*Names() вернут массив строк, в то время как остальные методы возвращают особые объекты-значения, содержащие информацю. Это наилучшим образом демонстрируется скриптом ниже.
