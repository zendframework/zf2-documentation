...
``Zend\Permissions\Acl`` предоставляет ``Zend\Permissions\Acl\Role`` в качестве базового класса,
...
Для этого примера мы используем ``Zend\Permissions\Acl\Role``, но можно было бы
