...
проинциализирован объект ``Zend\XmlRpc_Request\Http``, при этом данные
...
объект сообщения об ошибке ``Zend\XmlRpc_Server\Fault``. Эти объекты имеют
...
умолчанию в ``Zend\XmlRpc\Server`` тип запроса – ``Zend\XmlRpc_Request\Http``. Тем не
...
``Zend\XmlRpc_Response\Http``, который отправляет соответствующий XML-RPC
...
Для этого используйте *Zend\XmlRpc_Server\Fault::attachFaultException()* для
...
``Zend\XmlRpc_Server\Exception`` всегда находится в списке разрешенных
...
``Zend\XmlRpc_Server\Cache`` для кэширования определений сервера между
